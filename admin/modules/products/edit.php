<?php
    ...
    $name = array_get($_POST, 'name', $product['name']);
    $teaser = array_get($_POST, 'teaser', $product['teaser']);
    $content = array_get($_POST, 'content', $product['content']);
    $category_id = (int) array_get($_POST, 'category_id', $product['category_id']);
    $price = (int) array_get($_POST, 'price', $product['price']);
    $promotion_price = (int) array_get($_POST, 'promotion_price', $product['promotion_price']);
    $quantity = (int) array_get($_POST, 'quantity', $product['quantity']);
    ...
    if($action == 'update') {
        ...
        if(empty($errors)) {
            ...
            $affectedRows = $mProduct->update($data, ['id' => $productId]);
    ...
    <div class="form-group <?php echo isset($errors['category_id']) ? 'has-error' : '' ?>">
        ...
        <div class="col-sm-6">
            <select class="form-control" name="category_id">
                ...
                <option value="<?php echo $item['id'] ?>" <?php echo $item['id'] == $product['category_id'] ? 'selected' : '' ?>><?php for($i = 0; $i < $item['level']; $i ++) echo '--'; ?><?php echo $item['name'] ?></option>
                <?php endforeach; ?>
            </select>
            <?php if(isset($errors['promotion_price'])) : ?>
                <span class="help-block"><?php echo $errors['promotion_price'] ?></span>
